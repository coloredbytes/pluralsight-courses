
# Get deetails of loan
money_owed = float(input('How much money do you owe, in dollars"?\n')) # $50,000
apr = float(input('What is the annual percentage rate of the loan?\n')) #3%
payment = float(input('How much will ypu pay off in each month in dollars?\n')) # $1,0000
months = int(input('How many months do you want to see the results for?\n')) # 24

monthly_rate = apr/100/12

for i in range(months):
    # Calculate intrest ot pay
    interrest_paid = money_owed*monthly_rate

    # Add in interrest
    money_owed =  money_owed + interrest_paid

    if (money_owed - payment < 0):
        print('The last payment is', money_owed)
        print('You paid off thje loan in', i+1, 'months')
        break
    # Make payment
    money_owed = money_owed - payment

    print('Paid', payment, 'of which', interrest_paid, 'was interest', end=' ')
    print('Now I owe', money_owed)